		...
		_create : function() {
			...
			var $device = $(el).load("lib/html/device-widget.html", function(){
			    ...
			    var $status      = $el.find(".status-icon"),
			        ...
			        $temp        = $el.find(".temperature"),
			        $manage      = $el.find("button.manage");
			    var $holder = $el.find('.device-holder').data("id",dev.id);

			    // Set visibility on all fields
			    $title.toggle(show.title);
			    $description.toggle(show.description);
			    $([$status,$switch]).toggle(show.toggler);
			    $temp.toggle(show.temperature);

			    $title.html(dev.name);
			    $description.html(dev.description);
			    $temp.html(values[0] + " &deg;");
